...
the local database, they are marked as stranded.  When they exist in a
custom database, they are marked as foreign.
...
=item B<-r>, B<--repository> I<REPOSITORY>[B<,>[B<!>]I<REPOSITORY>]...
...
When the AUR is activated, packages identified as alternates are also
checked against the AUR to determine whether any of the alternate
packages are out of date with respect to the AUR.
...
package is from the local database, they are marked as C<stranded>; a
custom database, marked as C<foreign>.  When the package is from the
...
(yet) to the AUR in the case of C<stranded> packages, or that have been
...
The action needed fro a C<foreign> package is to place the source for
...
always mark such packages as C<foreign>.
